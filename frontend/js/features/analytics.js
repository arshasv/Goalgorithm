...
Router.register('analytics', () => {
  ...
  el.innerHTML = `
    <div class="page-header">
      ...
      <div class="page-header-actions">
        <select class="form-select" style="width:200px">
          <option>All Teams</option><option>Team A</option><option>Team B</option>
        </select>
      </div>
    </div>
    ...
    <div class="card section">
      <div class="card-header"><span class="card-title">🕸 Dimension Profile — Team A</span></div>
  ...
  // Per-match bars
  const perMatch=[{team:'A',base:25,grade:'A'},{team:'B',base:17,grade:'B'},{team:'C',base:15,grade:'B'},{team:'D',base:7,grade:'B'},{team:'E',base:0,grade:'C'}];
  document.getElementById('match-bars').innerHTML=perMatch.map((t,i)=>`
    <div style="display:flex;align-items:center;gap:var(--space-md);margin-bottom:var(--space-sm);animation:fadeInUp ${500+i*80}ms var(--ease-out) both">
      <span style="width:100px;font-size:var(--text-sm);text-align:right;font-weight:600;font-family:var(--font-display);text-transform:uppercase">Team ${t.team}</span>
