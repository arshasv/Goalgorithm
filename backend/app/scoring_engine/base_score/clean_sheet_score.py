def calculate_clean_sheet_score(
    ...
    both_correct_pts = config.get("clean_sheet_both_correct", 2.5) if config else 2.5
    one_correct_pts = config.get("clean_sheet_one_correct", 1.0) if config else 1.0
    none_pts = config.get("clean_sheet_none", 0.0) if config else 0.0
    ...
    if (home_pred_cs is None or away_pred_cs is None) and legacy_cs:
        ...
        if isinstance(h_val, bool):
            ...
            home_pred_cs = h_val >= 50.0 or (0.0 < h_val <= 1.0 and h_val >= 0.5)

        if isinstance(a_val, bool):
            away_pred_cs = a_val
        elif isinstance(a_val, (int, float)):
            away_pred_cs = a_val >= 50.0 or (0.0 < a_val <= 1.0 and a_val >= 0.5)
