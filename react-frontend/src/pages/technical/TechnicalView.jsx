...
const TechnicalView = () => {
  const [teams, setTeams] = useState(initialTeams.map(t => ({ ...t })));
  ...
  const handleReset = () => {
    setTeams(initialTeams.map(t => ({ ...t, code: 0, backend: 0, teamwork: 0, ai: 0 })));
  };

  const handleSubmit = async () => {
    if (!window.confirm('Submit Phase 2 technical scores for all 5 teams?')) return;
    for (const t of teams) {
      ...
      try {
        ...
        alert(`Failed for ${t.name}: ${e.message}`);
        return;
      }
    }
    ...
    <div>
      <div className="page-header">
        ...
        <div className="page-header-actions">
          ...
          <button className="btn btn-primary" onClick={handleSubmit}>✓ Submit All Technical Scores</button>
        </div>
      </div>
      <div className="alert alert-info">ℹ️ Enter integer scores 0–5 for each sub-dimension. Live total auto-calculates per team.</div>
